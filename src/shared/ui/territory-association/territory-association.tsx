        ...
        img.onload = () => {
          ...
          regions.forEach(region => {
            ...
            markerImg.onload = () => {
              ...
              const x = (region.coordinats.lng - mapBounds.minLng) / (mapBounds.maxLng - mapBounds.minLng) * canvas.width
              const y = (1 - (region.coordinats.lat - mapBounds.minLat) / (mapBounds.maxLat - mapBounds.minLat)) * canvas.height
              console.log(`Region: ${region.regionName}, x: ${x}, y: ${y}`)
              const markerWidth = 32
              const markerHeight = 48
              ctx.drawImage(markerImg, x - markerWidth / 2, y - markerHeight, markerWidth, markerHeight) // Размеры маркера можно изменить
            }
          })
        }
      }
